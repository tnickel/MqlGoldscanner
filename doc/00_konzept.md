# MqlGoldscanner — Konzept (v0.2.6, Stand 23.09.2026)
...
| **7 — Ausbau (S7)** ✅ | **Wochen-Summenwert** P(≥1 Bewegungstag; Modell 87 % vs. Klima 92 %, Unabhängigkeitsannahme dokumentiert), **Was-wäre-wenn-Simulation** (Vola-/Range-/Event-Slider auf den gespeicherten mu/sigma — klar gekennzeichnet, wird nie gespeichert), **REST-API** (read-only 127.0.0.1:8606, /status /matrix /prognose.csv /health, Sidebar-Badge, Port konfigurierbar). **Bewusst geprüft und geschoben:** GJR-GARCH/CARR (HAR deckt Clustering bereits ab, OOS-Nutzen unklar — Revisit wenn Track-Record systematische Vola-Verfehlung zeigt), Options-Skew/Termstruktur (Cboe-CDN-Rechte ungeklärt), Myfxbook-Outlook (Account nötig), WGC (Registrierung) | Ausbau nach Bedarf |
